...
- **被测项目作者：** @[最近一次推送的作者（如果不是 LuckyAnJun）]
- **测试框架维护者：** @LuckyAnJun
...
`;
              ...
              const issue = await github.rest.issues.create({
                ...
                body: issueBody,
                assignees: ['LuckyAnJun', commitAuthor].filter(Boolean),  // 指派给维护者和最近的非LuckyAnJun提交者
...
2. **Assignees（指派人）**
   - 自动指派给测试框架维护人员：**LuckyAnJun**
   - 同时指派给被测项目**最近一次推送的作者**（但如果该作者是 LuckyAnJun，则指派给更早的非 LuckyAnJun 提交者）
   - 获取方式：
     ```javascript
...
**测试框架在创建测试报告 Issue 前，必须先完成以下校验。校验失败时，测试框架 CI 必须失败，不能创建信息不完整的 Issue。**

1. **标题可追踪**
   - 必须符合 `[测试报告] 被测项目名 - 问题类型 - 简短描述`
   - 标题中的被测项目名必须与 `projects/` 下的 submodule 名称一致
   - 问题类型必须使用本文档定义的标准类型之一

2. **正文信息完整**
   - 必须包含被测仓库 URL、提交 SHA、提交作者、提交时间、分支、测试运行链接
   - 必须包含失败命令、失败摘要、关键错误日志或报告 artifact 链接
   - 必须包含复现步骤和验收标准
   - 如果缺少覆盖率、性能或安全扫描数据，必须写明 `N/A` 和原因，不能留空

3. **指派和标签准确**
   - 必须同时指派被测项目最近一次提交作者和测试框架维护者 `LuckyAnJun`
   - 必须包含 `test-report`、`auto-generated`、`needs-investigation`
   - 必须包含一个问题类型标签和一个优先级标签
   - 如果最近一次提交作者无法映射到 GitHub 用户，必须在 Issue 正文中记录原始作者信息，并至少指派 `LuckyAnJun`

4. **禁止重复创建**
   - 创建前必须搜索同一测试框架仓库中的 open Issue
   - 去重键必须至少包含：被测项目名、问题类型、提交 SHA、失败测试或失败命令
   - 如果已存在相同 open Issue，必须更新原 Issue 或追加评论，禁止再创建重复 Issue

5. **失败即阻断**
   - Issue 创建失败、字段校验失败、标签不存在且无法创建、指派失败时，测试框架 CI 必须标记失败
   - 禁止静默忽略 Issue 创建错误

6. **关闭条件明确**
   - 只有在重新运行测试并确认问题已修复后，才能关闭测试报告 Issue
   - 关闭评论必须包含通过的测试运行链接、修复提交或被验证提交、验证命令和结果摘要
   - 禁止只评论“已修复”“已完成”等无法追溯的说明
